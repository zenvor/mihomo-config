...
[3x-ui](https://github.com/zenvor/3x-ui) subconverter 模块使用的 Mihomo 订阅模板。
...
```
https://raw.githubusercontent.com/zenvor/mihomo-rules/main/mihomo.yaml
```

- 拉取使用 `If-Modified-Since` 条件请求，内容没变化时不重复下载。
...
两个占位符都必须出现在文件里，否则面板拒绝更新缓存（保留上一份可用模板）。
